...
import { formatDateTimeLocal, formatDateTimeLongLocal, toDateSafe } from '@/lib/dateUtils';
...
const TransactionsDashboard = () => {
  ...
  // Retourne la date actuelle en heure Haïti (America/Port-au-Prince)
  const getNowHaiti = () => {
    return new Date(new Date().toLocaleString('en-US', { timeZone: 'America/Port-au-Prince' }));
  };
  ...
  const isInTimeFilter = (createdAt) => {
    ...
    // Convertir le timestamp Supabase (UTC) en heure Haïti pour la comparaison
    const dtHaiti = new Date(
      toDateSafe(createdAt).toLocaleString('en-US', { timeZone: 'America/Port-au-Prince' })
    );

    if (timeFilter === 'today') return dtHaiti >= startOfToday();
    if (timeFilter === 'week') return dtHaiti >= startOfWeek();
    if (timeFilter === 'month') return dtHaiti >= startOfMonth();
    if (timeFilter === 'year') return dtHaiti >= startOfYear();
    ...
    if (timeFilter === 'custom') {
      ...
      if (from && dtHaiti < from) return false;
      if (to && dtHaiti > to) return false;
      return true;
    }
