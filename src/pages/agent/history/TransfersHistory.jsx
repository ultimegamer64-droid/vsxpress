...
import { formatDateTimeLocal, toDateSafe } from '@/lib/dateUtils';
...
const TransfersHistory = () => {
  ...
  const filterByTime = (item) => {
    ...
    // Convertir le timestamp Supabase (UTC) en heure Haïti
    const itemDateHaiti = new Date(
      toDateSafe(item.created_at).toLocaleString('en-US', { timeZone: 'America/Port-au-Prince' })
    );
    const nowHaiti = new Date(new Date().toLocaleString('en-US', { timeZone: 'America/Port-au-Prince' }));

    if (timeFilter === 'today') {
      return itemDateHaiti.toDateString() === nowHaiti.toDateString();
    }

    if (timeFilter === '7days') {
      const weekAgo = new Date(nowHaiti);
      weekAgo.setDate(nowHaiti.getDate() - 7);
      return itemDateHaiti >= weekAgo;
    }

    if (timeFilter === '30days') {
      const monthAgo = new Date(nowHaiti);
      monthAgo.setMonth(nowHaiti.getMonth() - 1);
      return itemDateHaiti >= monthAgo;
    }

    if (timeFilter === '1year') {
      const yearAgo = new Date(nowHaiti);
      yearAgo.setFullYear(nowHaiti.getFullYear() - 1);
      return itemDateHaiti >= yearAgo;
    }
    ...
    if (timeFilter === 'custom') {
      ...
      if (from && to) {
        ...
        return itemDateHaiti >= from && itemDateHaiti <= to;
      }

      if (from) return itemDateHaiti >= from;
      if (to) {
        to.setHours(23, 59, 59, 999);
        return itemDateHaiti <= to;
      }
    }
      ...
      >
        <div>
          ...
          <p className="text-sm text-[#A0A0A0]">
            {item.beneficiary_phone} • {opLabel} • {formatDateTimeLocal(item.created_at)}
          </p>
        </div>
