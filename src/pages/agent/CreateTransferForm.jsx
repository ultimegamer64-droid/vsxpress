...
const CreateTransferForm = () => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      // 1. Vérifications préliminaires (aucun argent touché encore)
      const status = await checkTransactionStatus(user.id);
      if (!status.allowed) throw new Error(status.message);

      // 2. Upload de la preuve EN PREMIER — avant tout débit
      // Si l'upload échoue ici, aucun argent n'a bougé
      ...
      const { data: funcData, error: funcError } = await supabase.functions.invoke('create-transfer', {
        body: {
          ...
          // Flag indiquant que le débit doit être fait dans cette même fonction
          include_debit: true,
        }
      });
      ...
      // Pas de rollback manuel nécessaire — l'Edge Function gère l'atomicité
      toast({
        variant: "destructive",
        title: t('common.error'),
